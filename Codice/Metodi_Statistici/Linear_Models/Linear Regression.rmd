...
cell_file <- "Codice/Metodi Statistici/lagged_datasets/Cella_15.csv"
...
createDFs <- function(df, partitions, partition_number = 1) {
  ...
  tr <- dbr[-unlist(partitions[partition_number]),]
...
```{r}
saveRDS(formula, file = "Codice/Metodi Statistici/saved parameters/Linear Regression/formula_ridotta_lr.rds")
```
...
errors <- rbind(errors,
                compute_metrics(name = paste0("Ridge Linear Regression (Lambda = ", bestlam,")"),
                                preds = predictions,
                                test = y_test
                )
)
...
```{r}
saveRDS(bestlam, file = "Codice/Metodi Statistici/saved parameters/Linear Regression/best_lambda_ridge.rds")
```
```{r}
library(ggplot2)
...
predictions <- unlist(as.data.frame(predictions))
errors <- rbind(errors,
                compute_metrics(name = paste0("Lasso Linear Regression (Lambda = ", bestlam,")"),
                                preds = predictions,
                                test = y_test
                )
)
...
```{r}
# lasso
saveRDS(bestlam, file = "Codice/Metodi Statistici/saved parameters/Linear Regression/best_lambda_lasso.rds")
```
...
```{r}
saveRDS(object = errors[,-1], file = "Codice/Metodi Statistici/saved parameters/metriche_errori/Regressioni_lineari.rds")
```
