...
class RewardFunction:
  ...
  def __init__(self, target_cell_temp, min_cell_temp,
    ...
    self.cell_temp_norm_term = max(max_cell_temp - target_cell_temp, target_cell_temp - min_cell_temp) # fattore di scala per calcolare la penalità associata ad una determinata temperatura

    self.max_glycol_temp = max_glycol_temp # solo la temperatura minima del glicole viene salvata, non ci serve salvare la temperatura massima
    self.glycol_range = max_glycol_temp - min_glycol_temp # fattore di scala per calcolare la penalità associata ad un determinato valore di temperatura del glicole
  ...
  def cell_temp_penalty(self, cell_temp):
    ...
    penalty = abs(cell_temp - self.target_cell_temp)/self.cell_temp_norm_term
    if isnan(penalty):
      print(f"Actual cell temp: {cell_temp}")
      print(f"Desired cell temp: {self.target_cell_temp}")
      print(f"Cell temp range: {self.cell_temp_norm_term}")
    return penalty
