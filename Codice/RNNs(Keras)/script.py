...
if __name__ == '__main__':
    ...
    # la rete neurale
    model = seq2seq_arch(Config.PAST_WINDOW_SIZE, Config.N_STATE_COLUMNS - 1, Config.FORECAST_WINDOW_SIZE,
                         n_labels=1, regression=False)
    # model = encoder_decoder_arch(Config.PAST_WINDOW_SIZE, Config.N_STATE_COLUMNS, Config.FORECAST_WINDOW_SIZE,
    #                              n_input_features=1, regression=True, whole_output_sequence=True, n_labels=1)
    ...
    for batch in test_data:
        ...
        for v in truth[:]:
            ...
            truth_flat.append(0 if v[Config.FORECAST_WINDOW_SIZE - 1][0] == 1 else 1)
        for v in pred[:]:
            # for i in range(Config.FORECAST_WINDOW_SIZE):
            #     pred_flat[i].append(v[i])
            pred_flat.append(0 if v[Config.FORECAST_WINDOW_SIZE - 1][0] > 0.5 else 1)
